...
objet::objet(objet_store* store, int id, float x, float y, float r, float v, float a, bool present) : id_(id), x_(x), y_(y), r_(r), v_(v), a_(a), z_(-1), present_(present), rayon_(0.01f + 0.001f*(rand() % 30)), comportement_(0), store_(store), couplage_(0), image_(cv::Size(640,480), CV_8UC1) {
